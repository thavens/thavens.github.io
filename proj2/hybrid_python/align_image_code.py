...
def rescale_images(im1, im2, pts):
    ...
    if dscale < 1:
        ...
        im2 = sktr.rescale(im2, 1.0 / dscale, channel_axis=2)
    return im1, im2
...
def match_img_size(im1, im2):
    ...
    assert im1.shape == im2.shape, (
        f"Images are not the same size after matching: {im1.shape} vs {im2.shape}"
    )
    return im1, im2
...
def align_images(im1, im2):
    ...
    print(f"im1s: {im1.shape}, im2s: {im2.shape}")
